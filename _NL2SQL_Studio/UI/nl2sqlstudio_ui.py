...
def define_post_auth_layout():
    ...
    def sidebar_components():
        ...
        with st.sidebar.container(height=60):
            st.session_state.execution = st.checkbox("Generate and Execute", disabled=False)
    ...
    def main_page():
        ...
        def help_info():
            with st.container():
                ...
                with column_3:
                    st.markdown('',
                                help="""For the purpose of this demo we have setup a demo
                                project with id 'sl-test-project-363109' created a dataset
                                in BigQuery named 'zoominfo'. This dataset contains 3 tables
                                with information that is a subset of Zoominfo Data Cubes.
                                This a the default dataset to generate SQLs from related natural
                                language statements.  For custom query generation, specify the
                                Project ID, Dataset and Metadata of tables in the Configuration
        ...
        def sample_queries_modal_active():
            ...
            if q_s_modal.is_open():
                with open('sample_questions.txt', 'r', encoding="utf-8") as input_file:
        ...
        def qa_modal_active():
            """
                Modal output when the Questions and Queries button on the Side bar
                panel is pressed
            """
            qa_modal = st.session_state.qa_modal
            if qa_modal.is_open():
