...
def linear_exec(question, execute_sql):
    ...
    resp = requests.post(url=end_point, data=json.dumps(data), headers=headers, timeout=None)
    # r = resp.json()
...
def proj_config(proj_name, dataset_name, metadata_filename):
    ...
    data = { "proj_name": proj_name, "bq_dataset": dataset_name, "metadata_file":metadata_filename }
    headers = {'Content-type': 'application/json', 'Accept': 'text/plain'}
    resp = requests.post(url=end_point, data=json.dumps(data), headers=headers, timeout=None)
    # r = resp.json()
