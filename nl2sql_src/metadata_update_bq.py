...
def generate_metadata(project_id, location, dataset_name, model_name="text-bison-32k", model_parameters=None):
    ...
    for table_ref in client.list_tables(dataset_name):
        ...
        if not table_description:
            prompt = f"Describe the data in table '{table_name}'. Do not include any markdown symmbols or special characters while generating the description"
        ...
        prompt += "\nPlease generate response in 'column name : Description' format. Do not include any markdown symmbols or special characters while generating the description"
...
if __name__ == "__main__":
    ...
    if PROJECT_ID is None or DATASET is None:
        print("Ensure you have set the PROJECT_ID and DATASET_NAME environment variables and retry")
    else:
